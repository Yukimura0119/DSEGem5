...
def parse_to_json(all_nodes):
    ...
    with open("../json/mobilenetv2.json", "w") as outfile:
        outfile.write(final)
...
def main():
    ...
    for i in input_all:
        if 'conv' in i.name or 'weight' in i.name:
...
#     insert conv tensor(weight, bias)
    ...
    for i in all_nodes:
        if i['op_type'] == 'Conv':
            weight_tensor = parse_tensor(conv_inputs[cnt])
            i['input_tensor'].append(weight_tensor)
            cnt += 1
