#ifndef _CONV_BATCHNROM_HPP_
#define _CONV_BATCHNROM_HPP_

#include "../include.hpp"
#include "../graph.hpp"
#include "./workload.hpp"

class ConvBatchnorm : public Workload
{
public:
    int numSubgraph = 1;
...
void ConvBatchnorm::split()
{
    ...
    // bias + batchnorm weight
    // split bias along output channel dimension
    if (split_node->inputs.size() == 7)
    {
        split_node->inputs[2]->shape[0] /= strategy.splitSize[1][0];

        if (strategy.splitSize[1][0] > 1)
        {
            for (int i = 3; i < split_node->inputs.size(); i++)
            {
                split_node->inputs[i]->shape[0] /= strategy.splitSize[1][0];
            }
        }
    }
    else
    {
        if (strategy.splitSize[1][0] > 1)
        {
            for (int i = 2; i < split_node->inputs.size(); i++)
            {
                split_node->inputs[i]->shape[0] /= strategy.splitSize[1][0];
            }
        }
